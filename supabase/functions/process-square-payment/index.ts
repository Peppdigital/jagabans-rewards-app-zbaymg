...
interface PaymentRequest {
  sourceId: string;
  amount: number;        // cents — total including tax + delivery
  subtotal: number;      // cents — item subtotal only
  taxAmount: number;     // cents — tax only
  currency?: string;
  customer: {
    name: string;
    email: string;
...
async function sendEmail(to: string, subject: string, html: string, text: string) {
  ...
  const restaurantEmail =
    Deno.env.get("RESTAURANT_EMAIL") || "noreply@jagabans.com";
...
// ─────────────────────────────────────────────────────────────
// Uber Direct dispatch — delegates to create-uber-delivery
// ─────────────────────────────────────────────────────────────
...
async function dispatchUberDelivery(
  ...
  // Build the flat delivery_address string for display/fallback
  ...
  // Build the Uber-structured address as a JSON string
  const uberAddress = {
  ...
  const data = await res.json();
  if (!res.ok) {
    throw new Error(data?.error ?? "Uber Direct dispatch failed");
  }
...
function buildCustomerEmailText(d: OrderEmailData): string {
  const deliveryLine =
    d.orderType === "delivery" && d.deliveryFee > 0
      ? `Delivery Fee: $${d.deliveryFee.toFixed(2)}\n`
      : "";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const subtotalDollars    = subtotal ? +(subtotal / 100).toFixed(2) : null;
    const taxDollars         = taxAmount ? +(taxAmount / 100).toFixed(2) : null;
    const deliveryFeeDollars =
      deliveryFeeCents != null ? +(deliveryFeeCents / 100).toFixed(2) : (body.deliveryFee ?? 0);
    ...
    const { data: paymentRow, error: payErr } = await supabaseAdmin
      ...
      .insert({
        ...
        metadata:          { customer, items, orderType, deliveryFee: deliveryFeeDollars } as unknown,
      })
    ...
    const emailData: OrderEmailData = {
      ...
      orderId:              orderRow.id,
      orderType,
      subtotal:             subtotalDollars ?? items.reduce((s, i) => s + i.price * i.quantity, 0),
      taxAmount:            taxDollars ?? 0,
      deliveryFee:          orderType === "delivery" ? deliveryFeeDollars : 0,
      total:                totalDollars,
      items,
