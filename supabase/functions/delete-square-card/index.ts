...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!sqResp.ok) {
      ...
      // Still remove from local DB even if Square call fails (card may already be disabled)
    }
    ...
    if (deleteError) {
      console.error("Failed to remove square_cards record:", deleteError);
      return json({ error: "Failed to remove card" }, 500);
    }

    // If the deleted card was default, promote the next most recent card
    if (card.is_default) {
