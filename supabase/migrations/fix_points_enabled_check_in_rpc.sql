-- Fix: create_order_from_payment must respect points_enabled from app_config.
-- Points were computed server-side from verified cents without ever checking
-- the toggle, so setting points_enabled = false had no effect.
...
BEGIN
  ...
  -- ── 5. Compute loyalty points — gated on points_enabled from app_config ───────
  --
  -- FIX: previously points were always computed from verified cents, ignoring
  -- the points_enabled toggle. Now we check app_config first. If the feature
  -- is disabled, points_earned is forced to 0 regardless of the order total.
  ...
  v_points_enabled := COALESCE(v_points_enabled, true);

  IF v_points_enabled THEN
    v_points_earned := FLOOR((v_subtotal_cents - v_discount_cents) / 100);
    RAISE NOTICE 'Points enabled — earning % points for payment %', v_points_earned, stripe_payment_intent_id;
  ELSE
    v_points_earned := 0;
    RAISE NOTICE 'Points disabled in app_config — no points awarded for payment %', stripe_payment_intent_id;
  END IF;
...
COMMENT ON FUNCTION create_order_from_payment(text) IS
  'Creates an order from a Stripe payment. Points are gated on points_enabled from app_config — the server-side toggle is authoritative and cannot be bypassed by client-supplied values.';
